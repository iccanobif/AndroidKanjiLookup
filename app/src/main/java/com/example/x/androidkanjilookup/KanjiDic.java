...
public class KanjiDic {
    ...
    public static void initialize(Context c) throws Exception
    {
        ...
        InputStreamReader f;
        if (c != null)
            f = new InputStreamReader(c.getAssets().open("kanjidic.txt"), "UTF8");
        else
            f = new InputStreamReader(new FileInputStream(System.getProperty("user.dir") + "\\src\\main\\assets\\kanjidic.txt"), "UTF8");
